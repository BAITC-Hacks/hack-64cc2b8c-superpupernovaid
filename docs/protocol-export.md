...
Экспорт требует сохранённый MeetingAnalysis. Публичный `/analyze` по-прежнему 501:
генерация/публикация анализа — отдельный этап. Готовность самого export module
проверяется synthetic fixture и HTTP-тестами; для реальных встреч без анализа
доступен корректный 409, upstream обработка автоматически не запускается.
